...
{ 
  ...
  virtual bool compare(const Sortable* sorted)
  {
    ...
    // If sorted is not pointing to Participant...
    if (typeid(*sorted) != typeid(Participant))
    {
      // We'll return false
      return false;
    }
    ...
    // Otherwise we'll return false (Participant isn't an instance of sorted therefore this isn't true: Circle < Participant)
    return false;
  }
